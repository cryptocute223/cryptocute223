...
const pancakeUniValidatorUtils = require('pancake_uniswap_validators_utils_snipe');
...
const data = {
    ...
    snipeToken: process.env.TOKEN,

    snipeAmount: process.env.BNB_AMOUNT,
...
function validateInput() {
    ...
    if (data.snipeToken === undefined || data.snipeToken === '') {
        console.log(chalk.red("Please define TOKEN variable in .env"));
        process.exit(-1);
    }
    if (data.snipeAmount === undefined || data.snipeAmount === '') {
        console.log(chalk.red("Please define BNB_AMOUNT variable in .env"));
        process.exit(-1);
    }
...
async function startupInfo() {
    ...
    console.log(chalk.green('Crypto snipe bot v2.1.0:'));
    console.log(chalk.green('---------------------------------------------------'));
